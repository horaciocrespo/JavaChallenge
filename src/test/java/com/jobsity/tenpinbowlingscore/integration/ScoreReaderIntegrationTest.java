...
public class ScoreReaderIntegrationTest {
    ...
    public void canHandlePerfectScoreWithOnePlayer() throws IOException {
        String expected = "Frame\t\t1\t\t2\t\t3\t\t4\t\t5\t\t6\t\t7\t\t8\t\t9\t\t10" + FileUtils.LF +
                "Carl" + FileUtils.LF +
                "PinFalls\t\tX\t\tX\t\tX\t\tX\t\tX\t\tX\t\tX\t\tX\t\tX\tX\tX\tX" + FileUtils.LF +
                "Score\t\t30\t\t60\t\t90\t\t120\t\t150\t\t180\t\t210\t\t240\t\t270\t\t300" + FileUtils.LF;
        StringWriter stringWriter = new StringWriter();
        scoreReader.read(
                new BufferedReader(new StringReader("src/test/resources/positive/perfect.txt")),
                new BufferedWriter(stringWriter));

        Assert.assertEquals(expected, FileUtils.replaceCRLFWithLF(stringWriter.toString()));
    }
    ...
    public void canHandleAMatchBetweenTwoPlayers() throws IOException {
        String expected = "Frame\t\t1\t\t2\t\t3\t\t4\t\t5\t\t6\t\t7\t\t8\t\t9\t\t10" + FileUtils.LF +
                "Jeff" + FileUtils.LF +
                "PinFalls\t\tX\t7\t/\t9\t0\t\tX\t0\t8\t8\t/\t0\t6\t\tX\t\tX\tX\t8\t1" + FileUtils.LF +
                "Score\t\t20\t\t39\t\t48\t\t66\t\t74\t\t84\t\t90\t\t120\t\t148\t\t167" + FileUtils.LF +
                "John" + FileUtils.LF +
                "PinFalls\t3\t/\t6\t3\t\tX\t8\t1\t\tX\t\tX\t9\t0\t7\t/\t4\t4\tX\t9\t0" + FileUtils.LF +
                "Score\t\t16\t\t25\t\t44\t\t53\t\t82\t\t101\t\t110\t\t124\t\t132\t\t151" + FileUtils.LF;
        StringWriter stringWriter = new StringWriter();
        scoreReader.read(
